/*
 * Sample 08 – NXP NTag 213 / 215 / 216 (.NET Framework 4.x)
 * ============================================================
 ...
 *
 * 사용법:
 *   NTag213.Net4x.exe COM3                      (Serial — Windows)
 *   NTag213.Net4x.exe pcsc                      (PC/SC — 첫 번째 리더 자동 선택)
 *   NTag213.Net4x.exe "pcsc:iksung IS-3500Z 0"  (PC/SC — 리더 이름 직접 지정)
 *
 * 플랫폼:
 * ┌──────────────────────────────────────────────────────────────────────────┐
                    ...
                    while (!cts.Token.IsCancellationRequested)
                    {
                        try
                        {
                            ...
                            try
                            {
                                ...
                                if      (chipType == "NTag215") userEnd = 129;
                                else if (chipType == "NTag216") userEnd = 225;
                                else                            userEnd = 39;
                            }
                            ...
                            try
                            {
                                ...
                                Console.WriteLine($"  ECC Signature      : {Hex(sig.Take(8).ToArray())}... ({sig.Length} bytes)");
                            }
                            catch (IksungProtocolException)
                            {
                            ...
                            Console.WriteLine($"\n  Fast Read pages {userStart}–{fastEnd} (first 16 pages of user area):");
                            try
                            {
                                ...
                                {
                                    byte[] page = fastData.Skip(i * 4).Take(4).ToArray();
                            ...
                            catch (IksungProtocolException)
                            {
                                ...
                                byte fallbackEnd = (byte)Math.Min(fastEnd, (byte)(userStart + 7));
                                for (byte p = userStart; p <= fallbackEnd; p++)
                                {
                                    try
                                    {
                            ...
                            // ── 6. 페이지 쓰기 (사용자 영역 첫 번째 페이지 = page 4) ──
                            {
                                ...
                                Console.WriteLine($"  Verify page {writePage}: {(match ? "PASS ✓" : "FAIL ✗")}  {Hex(verify)}");
                            }

                            /*
                             * ── 7. 패스워드 보호 (주석 해제 시 카드가 잠길 수 있음) ──
                             *
                             * byte[] pwd  = new byte[] { 0x01, 0x23, 0x45, 0x67 };  // 4-byte password
                             * byte[] pack = new byte[] { 0x89, 0xAB };               // 2-byte PACK
                             * await reader.NTagWriteAuth0Async(4, 1000, cts.Token);
                             * await reader.NTagWriteAccessAsync(0x80, 1000, cts.Token);

// =============================================================================
// Sample 15 – PC/SC (CCID) Channel  (.NET Framework 4.x)
// =============================================================================
...
// │  .NET Framework 4.7.2 이상에서 동작합니다.                                │
// └───────────────────────────────────────────────────────────────────────────┘
//
// 사용법:
//   (인수 없이 실행)                리더를 자동으로 찾습니다 — 포트를 몰라도 됩니다
//   PcscChannel.exe                         → 첫 번째 감지 리더에 자동 연결
//   PcscChannel.exe "iksung IS-3500Z 0"     → 지정 리더에 연결
// =============================================================================
...
namespace PcscChannel.Net4x
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            Console.WriteLine("=== Sample 15: PC/SC (CCID) Channel (.NET 4.x) ===");
            Console.WriteLine();
            Console.WriteLine("[A] 연결된 PC/SC 리더 목록 조회 중...");

            IReadOnlyList<string> readers = IksungPcscDiscovery.GetAvailableReaders();

            if (readers.Count == 0)
            {
                Console.WriteLine("    리더를 찾을 수 없습니다.");
                Console.WriteLine("    - USB CCID 리더가 연결되어 있는지 확인하세요.");
                Console.WriteLine("    - 리더가 CCID 모드일 때만 PC/SC 로 잡힙니다. ISReaderPro 에서 인터페이스를 CCID 로 바꾼 뒤 USB 를 다시 연결하세요.");
            ...
            if (args.Length > 0)
            {
                // 리더 이름에는 공백이 들어 있다(예: "iksung IS-3500Z 0").
                // 따옴표 없이 넘겨 이름이 잘려도 찾을 수 있도록 부분 일치로 고른다.
                string want = args[0];
                var hits = new List<string>();
                foreach (var r in readers)
                    if (r.IndexOf(want, StringComparison.OrdinalIgnoreCase) >= 0) hits.Add(r);
                if (hits.Count == 0)
                {
                    Console.WriteLine("[ERROR] \"" + want + "\" 와(과) 일치하는 리더가 없습니다.");
                    for (int i = 0; i < readers.Count; i++)
                        Console.WriteLine("    [" + i + "] " + readers[i]);
                    return;
                }
                readerName = hits[0];
                if (hits.Count > 1)
                    Console.WriteLine("[B] " + hits.Count + "개가 일치해 첫 번째를 사용합니다.");

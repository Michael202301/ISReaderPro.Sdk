// =============================================================================
// Serial (COM / USB-VCP) Channel  (.NET Framework 4.x)
// =============================================================================
...
//
// ┌─ 참고 ────────────────────────────────────────────────────────────────────┐
// │  .NET Framework(net472)는 Windows 전용이며 x86 으로 빌드됩니다.             │
// │  (net472 SerialPort 는 x64 에서 버그가 있어 x86 필수)                        │
...
namespace SerialChannel.Net4x
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            Console.WriteLine("=== Serial (COM/USB) Channel (.NET 4.x) ===");
            Console.WriteLine();
            ...
            if (port == null)
            {
                ...
                if (found.Count == 0)
                {
                    var all = new List<string>(IksungReaderDiscovery.GetAllSerialPorts());
                    Console.WriteLine("    리더가 붙은 시리얼 포트를 찾지 못했습니다.");
                    Console.WriteLine(all.Count == 0
                        ? "    이 PC에 시리얼 포트가 없습니다."
                        : "    이 PC의 시리얼 포트: " + string.Join(", ", all.ToArray()));
            ...
            catch (IksungException ex)
            {
                ...
                Console.WriteLine("    - 포트 이름이 맞는지 확인하세요 (장치 관리자 → 포트(COM & LPT)).");
